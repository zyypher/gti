...
const borderClassFor = (t: UiType) => {
    switch (t) {
        ...
        case 'banner_back':
            return 'border-purple-500' // class (plus inline fallback below)
        case 'advertisement':
...
const dotClassFor = (t: UiType) => {
    switch (t) {
        case 'banner_front':
            return 'bg-primary'
        case 'banner_back':
            return 'bg-purple-500' // class (plus inline fallback below)
        case 'advertisement':
            return 'bg-success'
        case 'promotion':
...
// Inline fallbacks in case your Tailwind theme doesn't include those palette keys
...
export default function NonProductPages() {
    ...
    const handleSubmit = async () => {
        ...
        formData.append('type', type) // banner_front | banner_back | advertisement | promotion
            ...
            {loading ? (
                <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                    {Array.from({ length: 6 }).map((_, i) => (
                        <div key={i} className="relative rounded border bg-white p-2 shadow-md">
                ...
                <div className="mx-auto grid max-w-6xl grid-cols-1 justify-center gap-4 px-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
                    {filteredItems.map((item) => {
                        ...
                        return (
                            <div
                                key={item.id}
                                className={`relative aspect-square w-full max-w-[250px] overflow-hidden rounded border-4 bg-white shadow-md ${borderClassFor(
                                    t
                                )}`}
                            ...
                            >
                                ...
                                {/* Title */}
                                <div className="absolute bottom-0 w-full truncate bg-white/90 px-1 py-1 text-center text-sm font-medium">
